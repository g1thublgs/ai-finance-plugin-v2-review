...
function buildRecordFromOcr(item = {}, index = 0) {
    ...
    return {
        recordKey: `meeting|${Date.now()}|${index}`,
        scenarioType: 'meeting',
        sourceRecognizeType: safeText(item.recognizeType || item.docType || item.type),
        title: safeText(firstValue(item, ['meetingName', 'title', 'subject', 'name', '项目名称'])),
        ...
        businessDate: safeText(firstValue(item, ['meetingDate', 'startDate', 'date', 'issueDate', '日期'])),
        participantCount: safeText(firstValue(item, ['attendeeCount', 'personCount', 'attendeeCount', 'traineeCount', 'guestCount', '人数'])),
        totalAmount: amount,
