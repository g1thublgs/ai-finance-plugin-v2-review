...
from meeting_common import make_fail, make_pass, make_skip, summary, find_keywords, build_evidence
...
def evaluate(context):
    location = summary(context).get('meetingLocation') or ''
    if not location:
        return make_skip('rule_01', RULE_META['name'], '会议地点字段', build_evidence(field='meetingLocation'))
    hits = find_keywords(location, FORBIDDEN)
    if hits:
        return make_fail('rule_01', RULE_META['name'], f'会议地点疑似位于禁止召开会议的风景名胜区：{location}', '请核对会议通知地点，必要时补充说明或调整会议地点。', build_evidence(location=location, hits=hits))
