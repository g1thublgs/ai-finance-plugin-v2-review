...
from meeting_common import make_pass, make_warning, payments, to_number, to_text, build_evidence

RULE_META = {'id': 'rule_02', 'name': '未使用公务卡结算提示', 'category': '公务卡结算', 'level': 'warning'}

def looks_person_name(name):
    text = re.sub(r'\s+', '', to_text(name))
    return bool(re.match(r'^[\u4e00-\u9fa5]{2,4}$', text)) and not any(k in text for k in ['公司', '酒店', '宾馆', '税务局', '中心', '单位'])
...
def evaluate(context):
    ...
    for row in rows:
        ...
        card_amount = to_number(row.get('cardAmount') or row.get('BX_JE') or row.get('刷卡金额'), 0)
        card_time = to_text(row.get('cardTime') or row.get('GWKHKSJ') or row.get('刷卡时间'))
        if payee and looks_person_name(payee) and not card_time and card_amount <= 0:
            issues.append(payee)
    if issues:
