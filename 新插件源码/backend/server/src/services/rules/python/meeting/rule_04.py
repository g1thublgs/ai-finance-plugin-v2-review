...
def evaluate(context):
    ...
    if missing:
        return make_warning('rule_04', RULE_META['name'], f'会议附件疑似不齐全，缺少：{"、".join(missing)}。', '请补充对应附件，或确认 OCR 材料类型识别是否准确。', build_evidence(missing=missing))
    return make_pass('rule_04', RULE_META['name'], '会议计划、会议通知、签到表/参会名单、费用明细/结算单均已识别。')
