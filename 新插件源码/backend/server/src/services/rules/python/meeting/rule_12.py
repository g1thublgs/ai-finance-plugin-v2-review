...
KEYWORDS = ['含酒精', '鱼翅', '燕窝', '茅台', '习酒', '郎酒', '五粮液', '泸州老窖', '剑南春', '洋河', '汾酒', '二锅头', '啤酒', '黄酒', '香烟', '中华', '云烟', '玉溪', '芙蓉王', '白沙', '黄鹤楼', '雪茄', '野生']
...
def evaluate(context):
    ...
    if hits:
        return make_warning('rule_12', RULE_META['name'], f'费用明细中出现需关注的高档菜肴、烟酒或野生等内容：{"、".join(hits)}。', '请核对该费用是否属于会议费合规开支范围。', build_evidence(hits=hits))
